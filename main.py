...
def draw_debug_info(renderer, detector, bot):
    ...
    bot_state: {bot.current_state}
    HP: {bot.enough_hp}
    MP: {bot.enough_mana}
    rebuffT: {int(bot.rebuff_time-time.time())}
    animusT: {int(bot.animus_last_see_time - time.time())}
    targetT: {int(bot.mob_ignore_timeout - time.time())}
    """
