...
class AggroMobsState(BotState):
    def handle(self, bot):
        aggroed_mobs = set()  # Для отслеживания уже сагренных мобов

        # Переключаемся на дальнобойное оружие
        if bot.current_weapon != "ranged":
            sleep(2)
            bot.switch_weapon("ranged")

        # Сортируем мобов по расстоянию
        bot.mobs.sort(key=lambda mob: mob.distance_to_character)

        for mob in bot.mobs:
            if mob.id in aggroed_mobs:
                bot.log(f"Skip mob {mob.id}")
                continue

            # Пытаемся выделить моба
            bot.select_target_by_coordinates(mob.get_center())
            sleep(1)

            if bot.have_target:
                bot.attack_target_until_hp_drops()
                aggroed_mobs.add(mob.id)
                bot.log(f"{bot.aggro_counter} {aggroed_mobs}")
                mob.aggroeds = True
                bot.aggro_counter += 1
                # Сбрасываем таргет
                bot.reset_target()
            ...
            if bot.aggro_counter >= cfg.MAX_AGGRO_MOBS:
                break

        if bot.aggro_counter >= cfg.MAX_AGGRO_MOBS:
            bot.aggro_counter = 0
            nearest_mob = min(bot.mobs, key=lambda mob: mob.distance_to_character)
            bot.select_target_by_coordinates(nearest_mob.get_center())
...
class AttackState(BotState):
    def handle(self, bot):
        ...
        if bot.current_weapon != "main":
            bot.switch_weapon("main")
        if not bot.have_target and not bot.have_close_targets_left:
